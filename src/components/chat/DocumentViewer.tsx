...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ file, extractedText }) => {
  ...
  if (!file) {
    return (
      <div className="text-sm text-muted-foreground">Upload a PDF, DOCX or TXT to preview here.</div>
    );
  }

  if (ext.endsWith(".pdf") && url) {
    return (
      <div className="rounded-xl overflow-hidden border border-border/50 h-[60vh] bg-background">
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-muted/40 p-4 h-[60vh] overflow-auto">
